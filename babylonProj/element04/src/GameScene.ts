...
import {
  ...
  export default function MenuScene(engine: Engine) {
    ...
    let title = createTextBlock(that.scene, "titleText","Welcome to Element 04",50,0,"white","0px","30px",advancedTexture);
    let description = createTextBlock(that.scene, "desc","This is an example of GUI in BabylonJS.",30,0,"green","0px","170px",advancedTexture);
    let description2 = createTextBlock(that.scene, "desc2","You can use the button below to return to Scene 1",30,0,"red","0px","200px",advancedTexture);

    let button1 = createSceneButton(that.scene, "but1", "Return Home","0px", "-200px", advancedTexture);    
    //--------------
    return that;
    //--------------
  }